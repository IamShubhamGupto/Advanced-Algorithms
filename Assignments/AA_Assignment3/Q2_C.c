    ...
    for(i = 1; i <= l2; i++){
        for(j = 0; j <= l1; j ++){
            if(j == 0){
                ...
                int a = get_position(text1[j-1]);
                int b = get_position(text2[i-1]);
                dp[i%2][j] = abs(a-b) + get_minimum(dp[(i - 1) % 2][j],get_minimum(dp[i % 2][j - 1], dp[(i - 1) % 2][j - 1]));
            }
        }
    }
    result = dp[l2 % 2][l1];
